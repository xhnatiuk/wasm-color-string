...
pub fn get_hwb(string: &str) -> Option<Hwb> {
    let re = Regex::new(r"^hwb\(\s*([+-]?\d{0,3}(?:\.\d+)?)(?:deg)?\s*,?\s*([+-]?[\d\.]+)%\s*,?\s*([+-]?[\d\.]+)%\s*(?:[,|/]\s*([+-]?[\d\.]+)\s*)?\)$").unwrap();
    let captures = re.captures(string)?;
    let h: f32 = match captures.get(1)?.as_str().parse::<f32>() {
        Ok(num) => num.rem_euclid(360.0),
        Err(_e) => return None,
    };
    let w: f32 = match captures.get(2)?.as_str().parse::<f32>() {
        Ok(num) => clamp(num, 0.0, 100.0),
        Err(_e) => return None,
    };
    let b: f32 = match captures.get(3)?.as_str().parse::<f32>() {
        Ok(num) => clamp(num, 0.0, 100.0),
        Err(_e) => return None,
    };
    let a: f32 = match captures.get(4) {
